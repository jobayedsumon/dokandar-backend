...
class PharmacyDriverOrderController extends Controller
{
use SendMail;
 use SendSms;
    public function pharmacy_dboy_completed_order(Request $request)
     {
     ...
     public function pharmacy_dboy_today_order(Request $request)
     {
         $date = date('Y-m-d');
        $delivery_boy_id = $request->delivery_boy_id;
        ...
        $ord =DB::table('orders')
             ...
             ->join('user_address', 'orders.address_id','=','user_address.address_id')
             ->join('delivery_boy', 'orders.dboy_id', '=','delivery_boy.delivery_boy_id')
             ->select('orders.order_status','orders.ui_type','orders.cart_id','tbl_user.user_name', 'tbl_user.user_phone', 'orders.delivery_date', 'orders.total_price','orders.delivery_charge','orders.rem_price','orders.payment_status','orders.payment_method','delivery_boy.delivery_boy_name','delivery_boy.delivery_boy_phone','orders.time_slot', 'vendor.vendor_loc as vendor_address', 'vendor.vendor_name','vendor.vendor_phone','vendor.lat as vendor_lat','vendor.lng as vendor_lng','user_address.lat as userlat', 'user_address.lng as userlng', 'delivery_boy.lat as dboy_lat', 'delivery_boy.lng as dboy_lng', 'user_address.user_number',  'user_address.houseno','user_address.pincode','user_address.houseno','user_address.street','user_address.state')
             ->where('orders.order_status','!=', 'completed')
             ->where('orders.ui_type','=', '3')
    ...
    public function pharmacy_dboy_nextday_order(Request $request)
    {
       ...
       $ord =DB::table('orders')
            ...
            ->where('orders.vendor_id','!=',0)
            ->where('orders.dboy_id',$delivery_boy_id)
            ->where('orders.delivery_date', $end)
            ->orderBy('orders.order_id', 'DESC')
            ->get();
    ...
    public function pharmacy_delivery_accepted_by_dboy(Request $request)
    {
     ...
     public function pharmacy_delivery_out(Request $request)
    {
       $cart_id= $request->cart_id;
       $ord = DB::table('orders')
            ->where('cart_id',$cart_id)
    ...
    public function pharmacy_delivery_completed(Request $request)
    {
        $created_at = Carbon::now();
       $cart_id= $request->cart_id;
    ...
    public function pharmacy_today_order_count(Request $request)
    {
        $date = date('Y-m-d');
        $delivery_boy_id = $request->delivery_boy_id;
